...
互动运行 14 天现在可选择先答问卷，并将问卷结果作为本局初始倾向。问卷独立入口仍保留。问卷结果只作为开局倾向，不作为永久人格定论，后续事件选择、NPC 反应和关系记忆仍会继续修正关系状态。

## v0.1.56 问卷题型与输入校验修正

问卷 Q024“被别人依赖”现在支持主反应 + 次反应，可以输入如 `1,5`。控制台输入提示更明确，会说明选项题支持数字或字母；多余逗号片段会被宽容忽略并提示玩家确认。

## v0.1.58 14 天互动玩法壳层 MVP

14 天互动流程新增普通日日常行动、NPC 简短回应、每日氛围状态头部、关键事件余波日和第 7 天中期反馈。普通日行动以氛围标签为主、轻量关系变化为辅；问卷初始倾向用于开局解释和少量文本染色。本版本不接 AI、不做自由输入、不做 UI、不做多 NPC、不重写底层关系系统。

## v0.1.59 即时反馈与逐日交互修复

互动模式现在会在玩家每天选择后立即显示当天反馈：普通日会立刻看到每日行动、NPC 回应、氛围变化和你的感受；关键事件日会立刻看到你的选择、记忆账本、关系状态变化和感知反馈。每天结束后会提示按回车进入下一天，最终仍保留完整 transcript 和第 14 天阶段结算。
