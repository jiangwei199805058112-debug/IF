...
开局关系设置拆分为“快速预设组合 / 根据问卷生成 / 手动自定义”。快速预设仍用于测试和快速试玩；根据问卷生成会复用本局问卷初始倾向；手动自定义先选择玩家倾向、NPC 倾向和主要矛盾主题，并映射到现有组合复用事件逻辑，不做完整角色生成。

第 14 天原因说明已增强，会综合多个关键事件和玩家行为模式，解释为什么进入继续暧昧、冷淡、危机或稳定推进。
